...
export class DesignerController {
  ...
  async deactivateTask(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ success: false, message: "Unauthorized" });
        return;
      ...
      const taskId = req.params.id as string;
      const task = await designerService.deactivateTask(taskId, req.user.id);
  ...
  async reactivateTask(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ success: false, message: "Unauthorized" });
        return;
      ...
      const taskId = req.params.id as string;
      const task = await designerService.reactivateTask(taskId, req.user.id);
