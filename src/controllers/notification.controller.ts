...
export class NotificationController {
  async getUserNotifications(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const viewedParam = req.query.viewed as string | undefined;
      const viewed = viewedParam !== undefined ? viewedParam === "true" : undefined;

      const result = await notificationService.getUserNotifications(userId, page, limit, viewed);
      res.json(result);
    } catch (err) {
