...
export class DataCollectorService {
  ...
  private buildPaginatedIdQuery(args: {
    ...
    if (assignedTo) conditions.push(`t.assigned_to_user_id = ${p(assignedTo)}`);
    ...
    const sql = `
      ...
      FROM data_collector_task t
      LEFT JOIN LATERAL (
        SELECT MAX(GREATEST(s.created_at, COALESCE(s.updated_at, s.created_at))) AS max_ts
        FROM data_collector_submission s
        WHERE s.data_collector_task_id = t.id
      ) sub ON true
      LEFT JOIN LATERAL (
        SELECT MAX(GREATEST(sr.created_at, COALESCE(sr.updated_at, sr.created_at))) AS max_ts
        FROM data_collector_submission s
        INNER JOIN data_collector_review sr ON sr.data_collector_submission_id = s.id
        WHERE s.data_collector_task_id = t.id
      ) subrev ON true
      ${where}
      ${orderBy}
  ...
  private buildCountQuery(args: {
    ...
    const where = conditions.length > 0 ? `WHERE ${conditions.join(" AND ")}` : "";
    const sql = `SELECT COUNT(*) AS count FROM data_collector_task t ${where}`;
  ...
  private async batchSubmissionsWithReviews(taskIds: string[], userId: string): Promise<Record<string, any>> {
    ...
    const submissions = await this.submissionRepo.find({
      where: { data_collector_task_id: In(taskIds) },
    });
    ...
    const submissionsByTask: Record<string, DataCollectorSubmission[]> = {};
    for (const s of submissions) {
      if (!submissionsByTask[s.data_collector_task_id]) {
        submissionsByTask[s.data_collector_task_id] = [];
      }
      submissionsByTask[s.data_collector_task_id].push(s);
    }
